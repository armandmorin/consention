...
BEGIN
  BEGIN
    DROP POLICY IF EXISTS "Allow full access to superadmins" ON public.global_settings;
  EXCEPTION
  ...
  BEGIN
    DROP POLICY IF EXISTS "Allow read access to admins" ON public.global_settings;
  EXCEPTION
    WHEN undefined_object THEN
      NULL;
  END;
